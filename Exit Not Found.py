...
class Monster:
    ...
    def find_path(self, tx, ty):
        ...
        while queue:
            ...
            for dx, dy in [(1, 0), (-1, 0), (0, 1), (0, -1)]:
                nx, ny = x + dx, y + dy
                if (nx, ny) not in came_from and self.can_move_to(nx, ny):
                    came_from[(nx, ny)] = (x, y)
                    if (nx, ny) == target:
                        queue.clear()
                        break
    ...
    def update(self, player_x, player_y, lever_active=False):
        ...
        if self.phase == PHASE_STALK:
            if dist < self.detection_range:
                ...
                if not self.at_corner:
                    self.move_along_path(player_x, player_y, self.stalk_speed)
            else:
                self.patrol()
                self.corner_spawn_timer = 0
                self.at_corner = False
        elif self.phase == PHASE_SUDDEN:
            self.move_along_path(player_x, player_y, self.hunt_speed)
        elif self.phase == PHASE_HUNT:
            self.move_along_path(player_x, player_y, self.hunt_speed)

    def draw(self, surface):
        """Draw monster with color based on phase"""
        color = MONSTER_COLOR
        if self.phase == PHASE_SUDDEN:
            color = (255, 100, 0)  # Orange for sudden
        elif self.phase == PHASE_HUNT:
            color = (255, 0, 0)    # Red for hunt

        pygame.draw.rect(surface, color, (self.x * TILE + MAZE_OFFSET_X, self.y * TILE + MAZE_OFFSET_Y, TILE, TILE))
...
def draw_sprite(screen, obj_x, obj_y, color, px, py, pa):
    dx = obj_x - px
    dy = obj_y - py
    ...
    while cur_dist < dist:
        ...
        if grid[int(ty)][int(tx)] == 1:
            return
    screen_x = MAZE_OFFSET_X + int((delta / HALF_FOV) * (VIEW_WIDTH / 2) + VIEW_WIDTH / 2)
    height = min(VIEW_HEIGHT, int(DIST_COEFF / (dist if dist > 0.1 else 0.1)))
    width = max(8, int(height * 0.5))
    pygame.draw.rect(screen, color, (screen_x - width // 2, 10 + VIEW_HEIGHT // 2 - height // 2, width, height))
...
def find_spawn_point(target_x, target_y, must_find=True):
    """Find nearest walkable point, ensuring it's within maze bounds and visible"""
    # Clamp target to valid maze bounds
    target_x = max(1, min(COLS - 2, target_x))
    target_y = max(1, min(ROWS - 2, target_y))
    
    candidates = []
    # Search only within valid maze area
    for y in range(max(0, target_y - 50), min(ROWS, target_y + 51)):
        for x in range(max(0, target_x - 50), min(COLS, target_x + 51)):
    ...
    # Fallback: search entire maze
    for y in range(ROWS):
        for x in range(COLS):
            if grid[y][x] == 0:
                return float(x), float(y)
    
    # Last resort: return clamped target
    return float(target_x), float(target_y)

# Player state with valid spawn point (ensure in bounds and visible)
player_spawn_x, player_spawn_y = find_spawn_point(20, 20)
...
monster_spawn_x, monster_spawn_y = find_spawn_point(COLS - 20, ROWS - 20)
monster = Monster(monster_spawn_x, monster_spawn_y)
is_running = False
lever_active = False
...
while running:
    ...
    # Update monster AI
    monster.update(player_x, player_y, lever_active)
    ...
    view_rect = pygame.Rect(MAZE_OFFSET_X, 10, VIEW_WIDTH, VIEW_HEIGHT)
    pygame.draw.rect(screen, MAZE_BORDER_BG, view_rect)
    pygame.draw.rect(screen, WALL, view_rect, 4)

    sky_color = (10, 16, 40)
    floor_color = (16, 16, 16)
    pygame.draw.rect(screen, sky_color, (MAZE_OFFSET_X, 10, VIEW_WIDTH, VIEW_HEIGHT // 2))
    pygame.draw.rect(screen, floor_color, (MAZE_OFFSET_X, 10 + VIEW_HEIGHT // 2, VIEW_WIDTH, VIEW_HEIGHT // 2))
    ...
    for ray in range(NUM_RAYS):
        ...
        x = MAZE_OFFSET_X + int(ray * SCALE)
        pygame.draw.rect(screen, wall_color, (x, 10 + VIEW_HEIGHT // 2 - line_height // 2, int(SCALE) + 1, line_height))
        if distance < 4 and ray % 18 == 0:
            blood_color = (180, 10, 20)
            red_line_y = 10 + VIEW_HEIGHT // 2 - line_height // 2 + int(line_height * 0.2)
            pygame.draw.line(screen, blood_color, (x, red_line_y), (x, red_line_y + int(line_height * 0.2)), 1)
            if ray % 36 == 0:
                spatter_y = red_line_y - 4
                pygame.draw.rect(screen, blood_color, (x - 1, spatter_y, 3, 3))

    # Small weapon/hands model
    gun_w = 60
    gun_h = 14
    gun_x = MAZE_OFFSET_X + VIEW_WIDTH // 2 - gun_w // 2
    gun_y = 10 + VIEW_HEIGHT - gun_h - 20
    pygame.draw.rect(screen, (40, 40, 40), (gun_x, gun_y, gun_w, gun_h), border_radius=6)
    pygame.draw.rect(screen, (100, 100, 120), (gun_x + gun_w // 2 - 6, gun_y - 10, 12, 10), border_radius=4)

    draw_sprite(screen, lever_x + 0.5, lever_y + 0.5, LEVER_COLOR, player_x, player_y, player_angle)
    draw_sprite(screen, monster.x + 0.5, monster.y + 0.5, (255, 0, 0), player_x, player_y, player_angle)

    if minimap_visible:
        # Draw centered minimap overlay
    ...
    if jumpscare_active and jumpscare_timer > 0:
        ...
        # Draw "MONSTER" text in huge distorted font
        jumpscare_font = pygame.font.Font(None, 120)
        scare_text = jumpscare_font.render("MONSTER!", True, (255, 255, 255))
